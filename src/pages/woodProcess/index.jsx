...
import CreateHumanProcess from "./CreateWoodProcess";

const WoodProcess  = () =>{
    const navigate  = useNavigate();
    const [currentPage,setCurrentPage] = useState(1);
    const [perPage, setPerPage] = useState(5);
    const attendanceHistory = useFetch(`wood?page=`+currentPage+'&pageSize='+perPage);
    ...
    const handleEdit = (row) => {
        navigate('/settings/wood-process/'+row.id);
    ...
    const handleDelete = (row) => {
        ...
        AxiosInstance.delete('wood/'+row.id).then(
            res=>{
    ...
    const handleActivate = (row) => {
        // Implement your delete logic here
        AxiosInstance.post('wood/set/'+row.id).then(
            res=>{
                attendanceHistory.fetchData(`wood?page=`+currentPage+'&pageSize='+perPage)
                toast(res.data.message);
    ...
    return (
        <div className="w-full flex justify-end text-gray-700 py-10 px-3 bg-gray-100 h-screen">
            ...
            <br />
            <ButtonLink to="/settings/wood-process/tambah" type="primary">Tambah Proses</ButtonLink>
            <br />
            <br />
            <DataTable
